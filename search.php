...
if (isset($_GET['search'])) {
    ...
    if ($order == 'like')
        ...
        $posts = Post::leftJoin('comments', 'posts.id', '=', 'comments.post_id')->select('posts.*')->selectRaw('COUNT(comments.id) as comment_count')->groupBy('posts.id');
    else
        $posts = Post::where('id', '!=', '0');
    //in
    if ($in == 'all')
        $posts = $posts->where('title', 'like', '%' . $word . '%')->orWhere('des', 'like', '%' . $word . '%');
    else if ($in == 'title')
        $posts = $posts->where('title', 'like', '%' . $word . '%');
    else if ($in == 'des')
        $posts = $posts->where('des', 'like', '%' . $word . '%');
    else
        $posts = $posts->where('title', 'like', '%' . $word . '%');
    //cat
    if ($cat)
...
//    print_r($posts->toSql());print_r($posts->getBindings());exit;
    //order
    if ($order == 'new')
        ...
        $posts = $posts->orderBy('comment_count', 'desc');
    $posts = $posts->get();
    ...
    <div class="blog_breadcrumb_wrapper" style="margin-top: 90px">
        <div class="container justify-content-center align-items-center">
            <div class="blog_breadcrumb_div">
                    <h3>جستجو برای [ <?= $word ?> ]</h3>
            </div>
        </div>
    </div>
    <div class="container" style="margin-top: 10px;margin-right: 140px">
        <p style="margin: 20px 0; font-weight: bold;font-size: 24px;color: black">فیلتر ها</p>
        <!--in-->
        <div style="display: flex; justify-content: start;align-items: center;gap: 10px;margin-right: 20px">
            <p style="padding-top: 15px;font-weight: bold;font-size: 18px;color: black">جستجو در : </p>
            <?php $in_url = $url; foreach ($_GET as $key => $value) { if ($key == 'in') continue; $in_url.= "$key=$value&";}?>
            <a href="<?= $in_url ?>in=title" class="btn btn-<?= isset($_GET['in']) && $_GET['in'] == 'title' ? 'secondary' : 'primary' ?>" type="text">عنوان</a>
            <a href="<?= $in_url ?>in=des" class="btn btn-<?= isset($_GET['in']) && $_GET['in'] == 'des' ? 'secondary' : 'primary'?>" type="text">توضیحات</a>
            <a href="<?= $in_url ?>in=all" class="btn btn-<?= isset($_GET['in']) && $_GET['in'] == 'all' ? 'secondary' : 'primary'?>" type="text">همه</a>
        </div>
        <!--cat-->
        <div style="display: flex; justify-content: start;align-items: center;gap: 10px;margin-right: 20px">
            <p style="padding-top: 15px;font-weight: bold;font-size: 18px;color: black">دسته بندی : </p>
            <?php $cat_url = $url; foreach ($_GET as $key => $value) { if ($key == 'cat') continue; $cat_url.= "$key=$value&";}?>
            <a href="<?= $cat_url ?>cat=all" class="btn btn-<?= isset($_GET['cat']) && $_GET['cat'] == 'all' ? 'secondary' : 'primary'?>" type="text"><?= 'همه' ?></a><div></div>
            <?php $categories = Category::where('subcat_id', 0)->get(); foreach ($categories as $category): ?>
                <a href="<?= $cat_url ?>cat=<?= $category->id ?>" class="btn btn-<?= isset($_GET['cat']) && $_GET['cat'] == $category->id ? 'secondary' : 'primary'?>" type="text"><?= $category->name ?></a>
                <?php $sub_Categories = Category::where('subcat_id', $category->id)->get(); foreach ($sub_Categories as $sub_Category): ?>
                    <a href="<?= $cat_url ?>cat=<?= $sub_Category->id ?>" class="btn btn-primary" style="background-color: <?= isset($_GET['cat']) && $_GET['cat'] == $sub_Category->id ? '#6c757d' : '#1482FFFF'?>;" type="text"><?= $sub_Category->name ?></a>
                <?php endforeach;?>
            <div></div>
            <?php endforeach;?>
        </div>
        <!--date-->
        <div style="display: flex; justify-content: start;align-items: center;gap: 10px;margin-right: 20px">
            <p style="padding-top: 15px;font-weight: bold;font-size: 18px;color: black">تاریخ : </p>
            <?php $date_url = $url; foreach ($_GET as $key => $value) { if ($key == 'date') continue; $date_url.= "$key=$value&";}?>
            <form method="get" class="d-flex">
                <input name="date" class="form-control" type="text" value="<?= $_GET['date'] ?? '' ?>" placeholder="<?= jdate('Y-m-d')?>">
                <input name="word" class="form-control" type="hidden" value="<?= $word ?>">
                <input name="cat" class="form-control" type="hidden" value="<?= $_GET['cat'] ?? '' ?>">
                <input name="order" class="form-control" type="hidden" value="<?= $_GET['order'] ?? '' ?>">
                <button name="search" class="btn btn-primary me-1" type="submit">اعمال</button>
            </form>
            <a href="<?= $date_url ?>date=<?= jdate('Y-m-d', tr_num:'en') ?>" class="btn btn-<?= isset($_GET['date']) && $_GET['date'] == jdate('Y-m-d', tr_num:'en') ? 'secondary' : 'primary'?> me-2" type="text">امروز</a>
        </div>
        <!--order-->
        <div style="display: flex; justify-content: start;align-items: center;gap: 10px;margin-right: 20px">
            <p style="padding-top: 15px;font-weight: bold;font-size: 18px;color: black">مرتب سازی : </p>
            <?php $order_url = $url; foreach ($_GET as $key => $value) { if ($key == 'order') continue; $order_url.= "$key=$value&";}?>
            <a href="<?= $order_url ?? $url ?>order=new" class="btn btn-<?= isset($_GET['order']) && $_GET['order'] == 'new' ? 'secondary' : 'primary'?>" type="text">جدید</a>
            <a href="<?= $order_url ?? $url ?>order=like" class="btn btn-<?= isset($_GET['order']) && $_GET['order'] == 'like' ? 'secondary' : 'primary'?>" type="text">محبوبیت</a>
            <a href="<?= $order_url ?? $url ?>order=view" class="btn btn-<?= isset($_GET['order']) && $_GET['order'] == 'view' ? 'secondary' : 'primary'?>" type="text">بازدید</a>
            <a href="<?= $order_url ?? $url ?>order=comment" class="btn btn-<?= isset($_GET['order']) && $_GET['order'] == 'comment' ? 'secondary' : 'primary'?>" type="text">کامنت</a>
        </div>
        <!--search-->
        <hr>
        <div style="display: flex; justify-content: center;align-items: center;gap: 10px;margin-right: 20px">
            <p style="padding-top: 15px;font-weight: bold;font-size: 24px;color: black">جستجو : </p>
            <form method="get" class="d-flex">
                <input name="date" class="form-control" type="hidden" value="<?= $_GET['date'] ?? ''?>">
